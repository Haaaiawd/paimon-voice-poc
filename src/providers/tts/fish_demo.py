"""TASK-008 人工验收入口：FishAudioTTS 真实合成 → StreamingPlayer 播放。

用法：

    python -m src.providers.tts.fish_demo            # 合成内置测句并播放
    python -m src.providers.tts.fish_demo --cancel   # 播到一半 cancel 再重开
    python -m src.providers.tts.fish_demo --out x.pcm  # 无声卡：落盘裸 PCM

每句打印 TTFA（stream_audio 调用 → 首字节音频）与总时长。
无输出设备时自动降级为不落盘只统计（或用 --out 落盘）。
...
def _load_api_key() -> str:
    ...
    key = (os.environ.get("FISH_API_KEY") or env.get("FISH_API_KEY") or "").strip()
    if not key:
        raise SystemExit(
            "[env] FISH_API_KEY 为空——在 .env 填入 Fish key 或导出同名环境变量"
        )
    return key
...
async def _synth_once(
    tts: FishAudioTTS, player, sink: list[bytes] | None
...
async def _run(args: argparse.Namespace) -> None:
    ...
    tts = FishAudioTTS(api_key=_load_api_key())
    ...
    if args.out is None:
        try:
            ...
            player = None
    print(
        f"[cfg] model={tts.model} format={tts.audio_format} "
        f"rate={tts.sample_rate} latency={tts.latency} proxy=None(direct)"
    )
    try:
        ttfa, total_ms, nbytes = await _synth_once(tts, player, sink)
        print(f"[tts] TTFA={ttfa:.0f}ms total={total_ms:.0f}ms bytes={nbytes}")
        if player is not None:
            ...
            async for audio in agen:
                ...
                if got == 1:
                    ...
                    if player is not None:
                        player.stop()
            post_cancel = got
            print(f"[cancel] chunks before stop={post_cancel}，重开验证…")
